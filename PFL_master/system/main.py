...
from flcore.servers.serverMetabayes import FedMetaBayes
from flcore.servers.serverpFedBayes import pFedBayes
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "mlr":
            ...
            if "mnist" in args.dataset:
                args.model = FedAvgCNN(in_features=1, num_classes=args.num_classes, dim=1152).to(args.device)
            elif "Cifar10" in args.dataset:
                args.model = FedAvgCNN(in_features=3, num_classes=args.num_classes, dim=2048).to(args.device)
        ...
        elif model_str == "bayes_cnn":
            if "Cifar10" in args.dataset:
                args.model = ER1FedAvgCNN(num_classes=args.num_classes, ens=args.num_ensemble, dim=2048,
                                      device=args.device).to(args.device)
            elif "mnist" in args.dataset:
                args.model = ER1FedAvgCNN(in_features=1, num_classes=args.num_classes, ens=args.num_ensemble, dim=1152,
                                          device=args.device).to(args.device)

        elif model_str == "lr_bayes_cnn":
            if "Cifar10" in args.dataset:
                args.model = LRFedAvgCNN(num_classes=args.num_classes, ens=args.num_ensemble, dim=2048, rank=args.rank,
                                      device=args.device, ada_rank=args.adaptive_rank, quan_method=args.quan_method).to(args.device)
            elif "mnist" in args.dataset:
                args.model = LRFedAvgCNN(in_features=1, num_classes=args.num_classes, ens=args.num_ensemble, dim=1152,
                                          device=args.device, ada_rank=args.adaptive_rank, quan_method=args.quan_method).to(args.device)
            ...
            args.model = BiLSTM_TextClassification(input_size=vocab_size, hidden_size=emb_dim,
                                                   output_size=args.num_classes, num_layers=1, embedding_dropout=0,
                                                   lstm_dropout=0, attention_dropout=0,
        ...
        elif args.algorithm == "FedMetaBayes":
            server = FedMetaBayes(args, i)
...
if __name__ == "__main__":
    ...
    algo = {0: ["bayes_cnn", "FedMetaBayes"],
            1: ["lr_bayes_cnn", "FedMetaBayes"],
            2: ["pFB_cnn", "pFedBayes"],
            3: ["cnn", "FedAvg"],
            4: ["cnn", "FedProx"],
            5: ["cnn", "Ditto"],
    ...
    dataset = ["Cifar10-dir-0.1", "Cifar10-pat-2N", "Cifar10-pat-5N", "Cifar10-pat-2S", "Cifar10-pat-5S",
            "Cifar10-pat-2M", "Cifar10-pat-5M", "Cifar100-pat-5S", "Cifar100-pat-5M"]
    ...
    parser.add_argument('-data', "--dataset", type=str, default=dataset[3])
    ...
    parser.add_argument('-eg', "--eval_gap", type=int, default=10, help="Rounds gap for evaluation")
    parser.add_argument('-jr', "--join_ratio", type=float, default=0.2, help="Ratio of clients per round")
    parser.add_argument('-rjr', "--random_join_ratio", type=bool, default=False, help="Random ratio clients per round")
    parser.add_argument('-nc', "--num_clients", type=int, default=50, help="Total number of clients")
    parser.add_argument('-nnc', "--num_new_clients", type=int, default=0)
    parser.add_argument('-fte', "--fine_tuning_epoch", type=int, default=20)
    ...
    parser.add_argument('-lrp', "--p_learning_rate", type=float, default=0.001, help="personalized learning rate")
    ...
    parser.add_argument('-cdr', "--client_drop_rate", type=float, default=0.0,
                        help="Rate for clients th at train but drop out")
    ...
    # FedMTL
    parser.add_argument('-itk', "--itk", type=int, default=4000, help="The iterations for solving quadratic subproblem")
    # FedAMP
    parser.add_argument('-alk', "--alphaK", type=float, default=1.0, 
                        help="lambda/sqrt(GLOBAL-ITERATION) according to the paper")
    parser.add_argument('-sg', "--sigma", type=float, default=1.0)
    # APFL
    parser.add_argument('-al', "--alpha", type=float, default=1.0)
    # Ditto / FedRep
    parser.add_argument('-pls', "--plocal_steps", type=int, default=20)
    # MOON
    ...
    parser.add_argument('-p', "--layer_idx", type=int, default=2, help="More fine-grained than its original paper.")
    # FedKD
    ...
    print("Ensemble number: {}".format(args.num_ensemble))
    print("Rank number: {}".format(args.rank))
    ...
    print("Fine tuning epochs on new clients: {}".format(args.fine_tuning_epoch))
    print("=" * 50)
